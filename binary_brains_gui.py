...
from binary_brains import translate_statement, validate_statement
import sys
...
class QuantifierGUI:
    ...
    def quit_app(self):
        ...
        try:
            ...
            image_path = os.path.join(script_dir, "goodbye_image.jpg")
            
            # Open and convert the image using PIL
            pil_image = Image.open(image_path)
            # Optionally resize the image if it's too large
            pil_image = pil_image.resize((300, 300))  # Adjust size as needed
            ...
            print(f"Error loading image: {e}")  # For debugging
    ...
    def show_help(self):
        help_text = """
        How to Use Binary Brains Translator:

        1. Enter a quantified statement in the input field
           Examples:
        ...
        3. The program will show two translations:
           - One for the general domain (All People)
           - One for the specific domain (Students)

        Tips:
        - Statements must include a quantifier (All, Some, No, None)
        - Clear button will reset all fields
        - Press Enter for quick translation
        
        Common Quantifiers:
        - Universal: All, Every, Each
        - Existential: Some, At least one
        ...
        help_window.configure(bg='#E6F3FF')
        
        # Add help text in a scrolled text widget
        ...
        help_text_widget.pack(padx=20, pady=20, fill='both', expand=True)
